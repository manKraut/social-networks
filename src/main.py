...
# Find directories which will hold the results at each step (if they exist) delete them and regenerate them
...
# open temporal network and retrieve all information needed
...
for i in range(N):
    dcntr, ccntr, bcntr, ecntr, kcntr = graphs.GraphToCsv(i, os.path.join(current_path, dir_paths[1])) \
        .centralities_to_csv()
    # Move generated files to the respective directories
...
for i in range(N - 1):
    similarities.Similarities((os.path.join(current_path, dir_paths[8]) + '\E_Star_G_' + str(i) + '_' + str(i) +
                               '.adjlist'), (os.path.join(current_path, dir_paths[8]) + '\E_Star_G_' + str(i) + '_' +
                                             str(i + 1) + '.adjlist'), pgd, pcn, pjc, pa, ppa).similarities()
